...
// Auto-inisialisasi aman: Buat activity_logs & superadmin bila belum ada (100% aman tanpa sentuh DPT/suara)
try {
...
try {
    $checkSuper = $pdo->prepare("SELECT id FROM users WHERE username = 'superadmin' LIMIT 1");
    $checkSuper->execute();
    if (!$checkSuper->fetchColumn()) {
        $superHash = password_hash('smksig123', PASSWORD_BCRYPT);
        $pdo->prepare("INSERT INTO users (username, password_hash, name, role) VALUES ('superadmin', ?, 'Super Administrator', 'superadmin')")->execute([$superHash]);
    }
} catch (Throwable $e) {
    // Ignore error
}
